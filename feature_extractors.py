...
class ASRSentiment(torch.nn.Module):
    ...
    def process_video(self, input_tensor=None, video_path=None, sr=None, **kwargs):
        ...
        # --- New Graph Embedding Pipeline ---
        import os
        import subprocess
        import torch

        # 1. Save the extracted text to a file
        os.makedirs("temp_asr", exist_ok=True)
        with open("temp_asr/asr_output.txt", "w") as f:
            f.write(text)

        # 2. Run txt_to_kg.py -> generates .edgelist
        subprocess.run(["python3", "KG/txt_to_kg.py", "temp_asr/asr_output.txt", "--output", "temp_asr/asr_output.edgelist"])

        # 3. Run edge_to_pyg.py -> generates .pt graph
        subprocess.run(["python3", "KG/edge_to_pyg.py", "temp_asr/asr_output.edgelist"])

        # 4. Run generate_embeddings.py -> outputs final embedding
        subprocess.run(["python3", "KG/generate_embeddings.py"])

        # 5. Load embedding tensor
        graph_embedding = torch.load("temp_asr/asr_output_node_embeddings.pt")
        graph_embedding = graph_embedding.mean(dim=0).unsqueeze(0)  # shape: (1, 768)
