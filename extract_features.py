...
for i, feature in enumerate(features):
    ...
    for video_path in tqdm(video_paths, mininterval=120, desc=f"{feature} - {len(features) - i} features remaining"):
        ...
        if feature == 'asr_sentiment':
            # Save both sentiment and graph embeddings separately
            sent_feat = video_output.get('features', None)
            graph_feat = video_output.get('graph_features', None)
            if not u.is_empty(sent_feat) and sent_feat is not None:
                x['samples'][video_name]['features']['asr_sentiment'] = u.detach_tensor(sent_feat)
            if not u.is_empty(graph_feat) and graph_feat is not None:
                x['samples'][video_name]['features']['asr_graph'] = u.detach_tensor(graph_feat)
        else:
            video_feature = video_output['features']
            if not u.is_empty(video_feature) and video_feature is not None:
                x['samples'][video_name]['features'][feature] = u.detach_tensor(video_feature)
...
genre_to_ind = {genre: ind for ind, genre in enumerate(all_genres)}
ind_to_genre = {ind: genre for genre, ind in genre_to_ind.items()}
